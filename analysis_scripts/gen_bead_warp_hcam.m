...
local_exp =  arg1;
%local_exp = 'C:\Users\Colenso\Desktop\GUI_XY_test\'
...
numbeadmov = numel(dir([acq_path 'Regbead_1_*.dax']));
if numbeadmov>1
...
track_file_nms1 = dir([base_pth 'Regbead_1_*750mlist.hdf5'] );
track_file_nms2 = dir([base_pth 'Regbead_1_*IR647mlist.hdf5'] );
track_file_nms3 = dir([base_pth 'Regbead_1_*Vis647mlist.hdf5'] );
track_file_nms4 = dir([base_pth 'Regbead_1_*561mlist.hdf5'] );
track_file_nms5 = dir([base_pth 'Regbead_1_*488mlist.hdf5'] );
...
for k = 1:len
    ...
    X_value = h5read(track_file_nm,'/fr_1/x');
    Y_value = h5read(track_file_nm,'/fr_1/y');
    ...
    X_value = h5read(track_file_nm,'/fr_7/x');
    Y_value = h5read(track_file_nm,'/fr_7/y');
    % resize x and y values for larger canvas
    ...
    X_value = h5read(track_file_nm,'/fr_10/x');
    Y_value = h5read(track_file_nm,'/fr_10/y');
    % resize x and y values for larger canvas
    ...
    X_value = h5read(track_file_nm,'/fr_14/x');
    Y_value = h5read(track_file_nm,'/fr_14/y');
    % resize x and y values for larger canvas
    ...
    X_value = h5read(track_file_nm,'/fr_21/x');
    Y_value = h5read(track_file_nm,'/fr_21/y');
    % resize x and y values for larger canvas
...
title(['561 to 647, no warping ' num2str(fac) 'x mag. residuals' ],...
    'fontsize',ti_fs)
hold off
xlim([-10 896*scale]); ylim([-20 896*scale])
