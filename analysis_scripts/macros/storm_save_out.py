...
#expfolder = "Z:\\Chenghang\\chenghaz001_4.10.2019_Pos_SampleA\\"
# set paths
...
for i in range(slicenum):
    for channel in channels:
        base = str(channel)
        if os.path.exists(ISanalysisfolder + "%04d" % i + "\\rawimages\\for_matlab\\" + base + "%02d" % i + ".tiff"):
            ...
            imp = IJ.openImage((storm_folder + base + "%02d" % i + "_mlist.tiff"))
            IJ.run(imp, "16-bit", "")
            imp.show()
