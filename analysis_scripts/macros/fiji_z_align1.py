...
conv_561_folder = exp_folder + "unaligned/conv_561/"
...
for i in range(num_sections):
	...
	for j in range(4):
		...
		#imp2.show()
		if patch_num == 0:
			IJ.saveAs(imp2, "TIF", (exp_folder + "rigid_align/storm_merged/" + basename[:-4] + ".tif"))
			IJ.saveAs(imp4, "TIF", (exp_folder + "rigid_align/storm_merged_ds/" + basename[:-4] + ".tif"))
		if patch_num == 1:
			IJ.saveAs(imp2, "TIF", (exp_folder + "rigid_align/conv_merged/" + basename[:-4] + ".tif"))
			IJ.saveAs(imp4, "TIF", (exp_folder + "rigid_align/conv_merged_ds/" + basename[:-4] + ".tif"))
			IJ.run("Close All", ""); 

		if patch_num == 2:
			IJ.run(imp2, "8-bit", "")
			IJ.saveAs(imp2, "TIF", (exp_folder + "rigid_align/conv_561/" + basename[:-4] + ".tif"))
			IJ.run(imp4, "8-bit", "")
			IJ.saveAs(imp4, "TIF", (exp_folder + "rigid_align/conv_561_ds/" + basename[:-4] + ".tif"))
			IJ.run("Close All", ""); 

		if patch_num == 3:
			IJ.run(imp2, "8-bit", "")
			IJ.saveAs(imp2, "TIF", (exp_folder + "rigid_align/for_align/" + basename[:-4] + ".tif"))
			IJ.run(imp4, "8-bit", "")
			IJ.saveAs(imp4, "TIF", (exp_folder + "rigid_align/for_align_ds/" + basename[:-4] + ".tif"))
			IJ.run("Close All", ""); 
